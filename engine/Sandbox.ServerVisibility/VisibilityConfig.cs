...
public static class VisibilityConfig
{
	...
	/// <summary>
	/// Randomization applied to grace period to prevent hacker prediction of culling time.
	/// Actual grace = CullGracePeriod + Random(0, CullGracePeriodRandomness).
	/// </summary>
	[ConVar( "sv_vis_grace_randomness", ConVarFlags.Protected,
		Help = "Random variance added to grace period (seconds)." )]
	public static float CullGracePeriodRandomness { get; set; } = 0.2f;

	/// <summary>
	/// Maximum distance (world units) an entity can teleport without invalidating cache.
	/// Beyond this, cache is treated as stale even within CacheValidTicks window.
	/// </summary>
	[ConVar( "sv_vis_teleport_threshold", ConVarFlags.Protected,
		Help = "Distance threshold for cache invalidation on teleport (world units)." )]
	public static float MaxTeleportDistance { get; set; } = 256f;
	...
	/// <summary>
	/// Enable audit logging for visibility decisions (owner bypasses, spectator claims, etc.).
	/// Logs are written to server console and output_log.txt.
	/// </summary>
	[ConVar( "sv_vis_audit_log", ConVarFlags.Protected,
		Help = "Enable security audit logging for visibility system." )]
