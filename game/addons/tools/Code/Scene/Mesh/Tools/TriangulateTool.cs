...
public partial class TriangulateTool( MeshFace[] faces ) : EditorTool
{
	...
	private readonly Dictionary<MeshComponent, List<Line>> _interiorEdgeLines = [];
	...
	private readonly Dictionary<MeshComponent, List<FaceHandle>> _triangulatedFaces = [];
	...
	{
		foreach ( (MeshComponent component, List<Line> lines) in _interiorEdgeLines )
		{
	...
	public void UpdateTriangulation( QuadMethod quadMethod, NgonMethod ngonMethod, int minTriangles )
	{
		...
		foreach ( (MeshComponent component, PolygonMesh originalMesh) in _originalMeshes )
		{
			...
			FaceHandle[] facesToTriangulate = _remappedFaces[component]
				.Select( f => faceMap.TryGetValue( f, out FaceHandle mapped ) ? mapped : default )
				.Where( f => f.IsValid )
				.ToArray();
			...
			foreach ( FaceHandle face in facesToTriangulate )
			{
				...
				{
					TriangulateQuad( TriangulateQuadMethod, mesh, face, out List<FaceHandle> newFaces,
				...
				{
					TriangulateNgon( TriangulateNgonMethod, mesh, face, out List<FaceHandle> newFaces,
	...
	public void Apply()
	{
			       ...
			       .Push() )
		{
			...
			foreach ( MeshComponent component in components )
			{
				...
				if ( !_triangulatedFaces.TryGetValue( component, out List<FaceHandle> triangulatedFaces ) )
				{
					continue;
				}

				foreach ( FaceHandle face in triangulatedFaces.Where( f => f.IsValid ) )
				{
	...
	private static void TriangulateQuad( QuadMethod method, PolygonMesh mesh, FaceHandle face,
	...
	private static void TriangulateNgon( NgonMethod method, PolygonMesh mesh, FaceHandle face,
		out List<FaceHandle> newFaces,
		out List<HalfEdgeHandle> newEdges )
	{
		newFaces = [];
